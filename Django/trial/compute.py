...
def compute(demand, isl, weekp, tr, iterations, combm, n, m, sc, st, rph, rpf, ma, kshift, kterm, kreset):
    first = 1
    total_warehouses = m
    total_weeks = n
    total_rakes = tr
    demand = charint(demand)
    initial_stock_level = charint(isl)
    storage_capacity = charint(sc)
    terminal_capacity = charint(st)           
    max_allottment = charint(ma)

    rake_penalty_h =rph;
    rake_penalty_f = rpf;

    weekly_penalty =   charint(weekp)
    weekly_penalty = weekly_penalty.reshape(8,4)

    k_shift = kshift
    k_reset = kreset
    k_terminate = kterm
    comb_matrix = charint(combm) 
    ...
    for i in range(0,1000):
      demand = np.array([3,3,2,2,1,1,3,3])
      ...
      if(allocated == total_rakes and first != 1):
        if(total_penalty<final_total_penalty):
            ...
            final_cup = capacity_utilization_penalty
            final_comb = comb 
    return(final_weekly_distribution),(final_total_penalty),(final_rake_penalty),(final_week_penalty),(final_cup),(final_comb)
    end = time.time()
#print(end-start)