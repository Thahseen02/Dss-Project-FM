...
dv1=floor(10*rand(1,1))+1;  //random number between 1 and 10 
...
while surplus>0 && dv1 > 5
        ...
        f1 = find (available>0);
        size_f1 = size(f1);
        rn1 = floor(size_f1(2)*rand(1,1))+1; //f1(2) gives the column number
        r_depot = f1(rn1);
...
for q = 1:num_excess
        ...
        while excess_qty_q >0                      //excess allocation is completed at this stage itself
...
while sum2 < sum(demand)
    ...
    half_rakes = sum (weekly_distribution==1);
    //half_rakes = half_rakes';      
    full_rakes = sum(weekly_distribution==2);
    //full_rakes=full_rakes';
    //size1 = size(half_rakes);
    //size2 = size(full_rakes);
    cost1 = half_rakes*rake_penalty_h;
    cost2 = full_rakes*rake_penalty_f;
    ...
    //cost4 = sum(cost3);
    //cost5=sum(cost4);
    week_penalty =sum(cost3);
    
    weekly_distribution_t = weekly_distribution';
    qty1 = sum(weekly_distribution_t,1);
